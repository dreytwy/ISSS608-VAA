...
```{r, echo=FALSE}
img <- image_read("https://r4va.netlify.app/chap27/img/image2.jpg")
img
```
...
```{r, echo=FALSE}
img <- image_read("https://r4va.netlify.app/chap27/img/image3.jpg")
img
```
...
Before getting started, we read these two articles:
...
# 5.5 Plotting Network Data with **ggraph** package
...
```{r, echo=FALSE}
img <- image_read("https://r4va.netlify.app/chap27/img/image4.jpg")
img
```

```{r, echo=FALSE}
img <- image_read("https://r4va.netlify.app/chap27/img/image5.jpg")
img
```
...
| Layout | Description |
|----|----|
| Stress | Default. Uses stress majorization for node arrangements to make geometric distances between nodes show their graph distances. |
| Sugiyama | Used for directed acyclic graphs (DAGs) that aims to minimize edge crossings and bends. |
| LGL (Large Graph Layout) | Designed for large graphs. Fast layout to handle networks with many nodes. |
| Graphopt | Force-directed algorithm that often produces compact layouts. |
| KK (Kamada-Kawai) | A spring-based layout that treats edges as springs and tries to find an equilibrium state that minimizes the total energy of the system. Often produces aesthetically pleasing layouts for smaller graphs. |
| Nicely | Layout that tries to pick a reasonable default layout based on the graph's characteristics. |
...
-   Dr. Kam Tin Seong (2025) [**Lesson 9: Visualising and Analysing Network Data**](%22https://isss608-ay2024-25apr.netlify.app/lesson/lesson09/lesson09-networkvis%22)
