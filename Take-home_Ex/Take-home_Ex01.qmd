...
Notice that: - Age is not a numerical value but a categorical value. - The Pop value is a summation for each combination of variables in a particular record. - In Time, there is only one year of data at 2024.
...
## 7.2 Pie Chart vs Stacked Bar Chart

### 7.2.1 Pie Chart of Gender by Population
...
### 7.2.2 Stacked Bar chart of Region by Population (with gender as hue)
...
-   The stacked bar chart with hover tooltip easily summarises the multiple piecharts of distribution of genders in regions in section 7.2.1.

## 7.3 Scatterplot vs Histogram

### 7.3.1 Scatterplot of Age by Gender
...
### 7.3.2 Histogram of Age by Population (with gender as hue)
...
## The Code for the Interactive Histogram
...
-   The histogram corroborates what was produced by the scatterplot in section 7.3.1.
...
Here, the graphical editor visualises, finds insights, plans, and rationalises the appropriate chart for the article in Section 9.

## 8.1 Population Pyramid (Age and Gender Structure)
...
#### Step 2: Creation of Empty Male and Female plots

::: panel-tabset
## Creation of women pyramid
...
#### Step 3: Plotting of Male + Female + Age Label plots

::: panel-tabset
## Plotting Females and Males
...
## 8.2 Histogram vs Scatterplot (Distribution of population by age stacked by gender, for different regions)
...
grid.rect(
  x = unit(0 + 0.01, "npc"),  # Move 1% of the width to the right
  y = unit(1 - 0.02, "npc"),  # Move 2% of the height down
  width = unit(0.05, "npc"),
  height = unit(0.025, "npc"),
  just = c("left", "top"),
  gp = gpar(fill = "#e5001c", lwd = 0)
...
## 8.3 Raincloud Plot (Distribution of weighted population by age-groups for different regions)
...
### 8.3.2 Plotting Raincloud Plots

::: panel-tabset
## Raincloud Plots by Gender
...
p9 <- ggplot(weighted_age_type, 
  ...
  labs(title = "Age Structure by Gender (Weighted Population)",
    ...
    legend.title = element_text(size = 6),
    legend.text = element_text(size = 6),
    plot.title=element_text(size=8, face = "bold", vjust=2),
    strip.text = element_text(size = 8)
  )+
...
## Code for Raincloud Plots by Gender
...
## Code for Raincloud Plot by Region
...
```{r, echo=FALSE}
combined<- (rcp1+ rcp2) +plot_annotation(
    caption = "Source: Department of Statistics, Singapore | Designer: AT")
...
##### In this article, we will cover the population pyramid, scatterplots, and raincloud plots. These are useful to visualise the demographic structures and distributions of a country.
...
::: card-body
#### **What is the regional distribution from the scatterplot?** {.card-title}
...
::: card-body
#### **What are the Density, Statistics, Outliers?** {.card-title}
