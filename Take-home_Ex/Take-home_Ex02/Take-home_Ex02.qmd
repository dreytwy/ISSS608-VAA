...
alluvial_data <- name_mapping %>%
  ...
  mutate(
    real_identity_wrapped = str_trunc(real_identity, 30) %>% str_wrap(10),
    observed_name_wrapped = str_trunc(observed_name, 30) %>% str_wrap(10),
    community_wrapped      = str_trunc(community, 30) %>% str_wrap(10),
    
    tooltip = paste0(
      "<b>Real Identity:</b> ", real_identity, "<br>",
      "<b>Observed Name:</b> ", observed_name, "<br>",
      "<b>Community:</b> ", community, "<br>",
      "<b>Count:</b> ", value
    )
  )

# --- 3. Create static ggplot object (MODIFIED) ---
p <- ggplot(alluvial_data,
            aes(axis1 = real_identity_wrapped, 
                axis2 = observed_name_wrapped, 
                axis3 = community_wrapped,
                y = value,
                text = tooltip)) +
  geom_alluvium(aes(fill = real_identity_wrapped), width = 0.95, alpha = 0.8) +
  geom_stratum(width = 0.95, fill = "grey90", color = "black", 
               aes(label = after_stat(stratum))) +
  geom_text(stat = "stratum", aes(label = after_stat(stratum)), size = 2)+  # Text in grey boxes
  scale_x_discrete(
    limits = c("Real Identity", "Observed Name", "Community"),
    expand = expansion(mult = c(0.1, 0.7), add = c(1, 1))
  ) +
  labs(
    title = "All Identity Flows (Interactive)",
    x = NULL, y = "Link Count"
  ) +
  theme_minimal() +
  theme(
...
# --- 3. Convert to interactive Plotly plot ---
interactive_plot <- ggplotly(p, tooltip = "text", width = 3000, height = 1000)
